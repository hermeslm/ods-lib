...
$templateCache.put('forms/schema/plugins/canvas-painter/canvas-painter-properties.html','<uib-tabset class="nav-tabs"><uib-tab index="0" heading="Properties"><form name="fieldPropertiesForm" class="form-horizontal"><div class="box-body padding-top"><div ng-include="\'forms/schema/components/base-properties/common-properties.html\'"></div><div ng-include="\'forms/schema/components/base-properties/text-properties.html\'"></div></div></form></uib-tab><uib-tab index="1" heading="Colors"><form name="fieldColorsForm" class="form-horizontal"><div class="box-body padding-top"><ods-canvas-painter-colors colors="field.options.lineColors"></ods-canvas-painter-colors></div></form></uib-tab><uib-tab index="2" heading="Debug" ng-show="debugMode"><div ng-include="\'forms/schema/components/base-properties/model-properties.html\'"></div></uib-tab></uib-tabset>');
...
$templateCache.put('forms/common/fields/plugins/canvas-painter/canvas-painter.html','<div ng-if="!field.readonly" ods-canvas-painter-color-selector="field.options.lineColors" selected-color="field.options.color" ng-style="{ width: field.options.width }"></div><input type="range" min="1" max="50" class="lineWidthSelector" ng-model="field.options.lineWidth" ng-if="!field.readonly" ng-style="{ width: field.options.width }"><div class="row" ng-if="!field.readonly"><div class="col-md-12">{{ field.options.lineWidth }}</div></div><div class="row"><div class="col-md-12"><button class="btn btn-success" ng-click="toggleEdit()">{{ field.readonly ? \'Enable Edit\' : \'Finish Edit\' }}</button> <button ng-if="!field.readonly" class="btn btn-warning" ng-click="undoCanvas()" ng-disabled="version === 0">Undo(Version {{version}})</button> <button ng-if="!field.readonly" class="btn btn-danger" ng-click="clearCanvas()" ng-disabled="version === 0">Clear</button></div></div><canvas ng-show="!field.readonly" id="ods-canvas" class="odsCanvasPaint" style="position:absolute"></canvas><canvas ng-show="!field.readonly" id="ods-canvas-tmp" class="odsCanvasPaint" style="position:relative;top: 0;left: 0"></canvas><figure ng-if="field.readonly" class="figure"><img id="ods-canvas-paint-viewer-result" ng-src="{{ field.value ? field.value : field.options.imageSrc }}" width="field.width" height="field.height"></figure>');
$templateCache.put('forms/common/fields/plugins/canvas-painter/color-selector.html','<ul class="odsCanvasSelector"><li ng-repeat="color in colorList track by $index" class="odsCanvasColor" ng-class="{\'active\': (selectedColor === color)}" ng-style="{\'background-color\':color}" ng-click="setColor(color)"></li></ul>');
